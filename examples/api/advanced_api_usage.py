...
def export_model_configs(output_file: str):
    ...
    console.print(f"[green]Exported {len(models)} model configurations to {output_file}[/green]")


def launch_with_custom_config(model_name: str, custom_options: Dict[str, Union[str, int, bool]]):
    ...
    console.print(f"[green]Model launched successfully![/green]")
...
def monitor_with_rich_ui(job_id: str, poll_interval: int = 5, max_time: int = 1800):
    ...
    with Progress() as progress:
        # Add tasks
        status_task = progress.add_task("[cyan]Waiting for model to be ready...", total=None)
        ...
        while elapsed < max_time:
            ...
            try:
                ...
                if status.status == ModelStatus.READY:
                    progress.update(status_task, description=f"[green]Model is READY at {status.base_url}[/green]")
                    break
                elif status.status == ModelStatus.FAILED:
                    progress.update(status_task, description=f"[red]Model FAILED: {status.failed_reason}[/red]")
                    break
                elif status.status == ModelStatus.PENDING:
                    progress.update(status_task, description=f"[yellow]Model is PENDING: {status.pending_reason}[/yellow]")
                elif status.status == ModelStatus.LAUNCHING:
                    progress.update(status_task, description="[cyan]Model is LAUNCHING...[/cyan]")
                elif status.status == ModelStatus.SHUTDOWN:
                    progress.update(status_task, description="[red]Model was SHUTDOWN[/red]")
                    break
            except Exception as e:
                progress.update(status_task, description=f"[red]Error checking status: {str(e)}[/red]")
...
def batch_inference_example(base_url: str, model_name: str, input_file: str, output_file: str):
    ...
    console.print(f"[green]Processed {len(inputs)} inputs and saved results to {output_file}[/green]")


def main():
    """Main function to parse arguments and run the selected function."""
    parser = argparse.ArgumentParser(description="Advanced Vector Inference API usage examples")
    subparsers = parser.add_subparsers(dest="command", help="Command to run")
    
    # Export configs command
    export_parser = subparsers.add_parser("export-configs", help="Export all model configurations to a JSON file")
    export_parser.add_argument("--output", "-o", default="model_configs.json", help="Output JSON file")
    
    # Launch with custom config command
    launch_parser = subparsers.add_parser("launch", help="Launch a model with custom configuration")
    ...
    launch_parser.add_argument("--max-model-len", type=int, help="Maximum model context length")
    launch_parser.add_argument("--max-num-seqs", type=int, help="Maximum number of sequences")
    ...
    # Monitor command
    monitor_parser = subparsers.add_parser("monitor", help="Monitor a model with rich UI")
    ...
    monitor_parser.add_argument("--max-time", type=int, default=1800, help="Maximum time to monitor in seconds")
    ...
    metrics_parser.add_argument("--duration", type=int, default=60, help="Duration to stream metrics in seconds")
    metrics_parser.add_argument("--interval", type=int, default=5, help="Polling interval in seconds")
    ...
    batch_parser.add_argument("--input", "-i", required=True, help="Input file with one prompt per line")
    batch_parser.add_argument("--output", "-o", required=True, help="Output JSON file for results")
    ...
    if args.command == "export-configs":
        ...
        if status.status == ModelStatus.READY:
            if console.input("[cyan]Stream metrics for this model? (y/n): [/cyan]").lower() == "y":
